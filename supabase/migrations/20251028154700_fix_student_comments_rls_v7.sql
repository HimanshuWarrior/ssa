...
CREATE POLICY "view_comments_policy" 
  ...
  USING (
    ...
    (
      student_id = CAST(CAST(auth.uid() AS text) AS uuid)
    )
    OR
    ...
    (
      EXISTS (
        ...
        WHERE id = CAST(CAST(auth.uid() AS text) AS uuid)
      )
      AND
      EXISTS (
        SELECT 1 
        ...
        WHERE tcs.teacher_id = CAST(CAST(auth.uid() AS text) AS uuid)
        AND s.id = student_id
      )
    )
    OR
...
CREATE POLICY "insert_comments_policy"
  ...
  WITH CHECK (
    ...
    commented_by = CAST(CAST(auth.uid() AS text) AS uuid)
    AND
    (
